...
Both cogs also cache display names keyed by ID - `voicelog` maintains `user_names`, `channel_names`, and `guild_names`; `gamelog` maintains its own `user_names` and `guild_names` - since the session tables above only ever store raw Discord IDs, and anything reading this data directly (rather than through Red/discord.py, which resolves names from its own live cache) needs a name to show. Each is refreshed opportunistically from whichever member/channel object is already in hand when a session starts or stops, not a separate lookup, so there's no meaningful extra cost. Both cogs also backfill every currently-known member (and, for `voicelog`, every voice/stage channel) once on load/reload, so anyone already connected or mid-game before the cache existed gets a name immediately rather than waiting on their next join/leave/move or game start/stop. Both `user_names` tables are pruned by `red_delete_data_for_user` alongside the session rows.
